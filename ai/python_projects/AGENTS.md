...
- if writing more languages in the project as a whole, also apply best practices relevant to this language; for example:
  - if writing and running bash scripts, use `shellcheck` (either already installed or `mamba` installable from conda-forge) and run it on the script, and use best practices (headers to fail on first error etc)
  - if writing and running bash scripts, use the preamble described in https://raw.githubusercontent.com/jerabaul29/config_scripts_snippets/refs/heads/main/snippets/bash/preamble/example_preamble.sh ; bash / shell makes it easy to shoot oneself in the foot, make sure to avoid that!
